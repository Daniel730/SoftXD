...
$id = $_GET['id'];
$sql = mysqli_query($conexao, "SELECT * FROM logs WHERE id='$id'");
while($row = mysqli_fetch_assoc($sql)){
    $tipoVar = $row['tipoVar'];
    $processo = $row['processo'];
    $nomeVar = $row['nomeVar'];
    $nomeFreq = $row['nomeFreq'];
    $medSep = $row['medSep'];
    $valMedSep = $row['valMedSep'];
    $ordem = $row['ordemOrdinal'];
    $dados = $row['dados'];
    $data = $row['data'];
}
// $sql = mysqli_query($conexao, "INSERT INTO logs(ordemOrdinal, nomeVar, nomeFreq, tipoVar, medSep, valMedSep, dataAnalise, dado) VALUES('$ordem', '$nomeVar', '$nomeFreq', '$tipoVar', '$medSep', '$valMedSep', '$data', '$dados')");
